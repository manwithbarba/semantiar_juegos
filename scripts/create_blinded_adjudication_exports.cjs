...
const experimentId = String(source.experimentId || 'semantiar-study');
const blindQueueVersion = 'annotator_blind_v2';
const blindId = (record) => 'BLD-' + crypto.createHash('sha256').update(experimentId + '\0' + String(record.discrepancyId)).digest('hex').slice(0, 20);
...
const records = (source.records || []).map((record) => ({
  blindRecordId: blindId(record), noteType: record.noteType || null, kind: record.kind || null, priority: record.priority || null,
  clinicalText: record.clinicalText || '', textSha256: record.textSha256 || null,
  annotatorA: blindAnnotator(record.annotatorA, 'A'), annotatorB: blindAnnotator(record.annotatorB, 'B'),
  mismatchFields: record.mismatchFields || [], alignment: record.alignment || null, thirdDecision: record.thirdDecision || null,
  blindQueueVersion
}));
const safeDocument = { schemaVersion: 'adjudication-input-blind.v2', experimentId, protocolVersion: source.protocolVersion || null, task: source.task || null, blindQueueVersion, instructions: source.instructions || null, records };
const traceMap = { schemaVersion: 'adjudication-blind-trace-map.v1', experimentId, blindQueueVersion, generatedAt: new Date().toISOString(), records: (source.records || []).map((record) => ({ blindRecordId: blindId(record), sourceDiscrepancyId: record.discrepancyId, sourceCaseId: record.caseId, sourceQueueRow: record.queueRow, sourcePair: record.pair, sourceTextSha256: record.textSha256 || null })) };
...
const manifest = { schemaVersion: 'adjudication-blind-manifest.v1', experimentId, blindQueueVersion, generatedAt: new Date().toISOString(), recordCount: records.length, jsonlSha256: crypto.createHash('sha256').update(jsonl).digest('hex'), jsonlFile: path.basename(jsonlFile), groupedJsonFile: path.basename(groupedFile), traceMapFile: path.basename(traceFile), traceMapScope: 'PI-only; never distribute to annotators' };
